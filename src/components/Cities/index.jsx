...
export default function MyCities() {
    ...
    const unbookmarkHandler = (cityName, cityId) => {
        const confirmed = confirm(
            `You wanna delete "${cityName}" city from your favorites list?`
        );
        if (!confirmed) return;
        globalDispatch({
    ...
    if (!favCities.length) return <FavoritesEmpty t={t} />;
                    ...
                    return (
                        <div className={styles.cityCard} key={data.id}>
                            <div className={styles.cityName}>
                                {data.name} <br />
                                <span>
                                    {data.sys.country in countriesCode
                                        ? countriesCode[data.sys.country][language]
                                        : "unknown"}
                                </span>
                            </div>
                            ...
                            <div className={styles.action}>
                                <button onClick={() => seeDetailsHandler(data.coord)}>
                                    {t("favPage_seeDetails")}
                                </button>
                                <button onClick={() => unbookmarkHandler(data.name, data.id)}>
                                    ...
                                    >
                                        <path
                                            fill="currentColor"
                                            d="m16 16.707l1.146 1.147a.5.5 0 0 0 .708-.708l-15-15a.5.5 0 1 0-.708.708L4 4.707V17.5a.5.5 0 0 0 .794.404L10 14.118l5.206 3.786A.5.5 0 0 0 16 17.5zM16 4.5v9.379L4.794 2.673A2.5 2.5 0 0 1 6.5 2h7A2.5 2.5 0 0 1 16 4.5"
                                        ></path>
                                    </svg>
                                </button>
                            </div>
...
/**
 * when there is no favorites
 */

function FavoritesEmpty({ t }) {
    return (
        <section className={styles.favCitiesSection}>
            <div className={styles.emptyIcon}>
                ...
                >
                    <path
                        ...
                        strokeWidth={1.5}
                        d="M12 11.5v5m0-8.99l.01-.011M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2S2 6.477 2 12s4.477 10 10 10"
                    ></path>
                </svg>
            </div>
            <div className={styles.emptyText}>{t("favPage_empty")}</div>
        </section>
    );
}
